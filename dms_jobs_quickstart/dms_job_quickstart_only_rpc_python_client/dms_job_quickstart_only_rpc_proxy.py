#------------------------------------------------------------------
# Proxy Script Generated by Delphi Microservices Container 4.1.45
# Do not modify this code!
# Generated at: 2021-11-25 12:07:23
#------------------------------------------------------------------
...
class JobQuickStart1RPCException(Exception):
    pass

class JobQuickStart1RPCProxy:
    ...
    def __execute(self, req):
        ...
        if res.status_code == 204:  # no content
            if isnotif:
              ...
              raise JobQuickStart1RPCException(0,"Protocol error for Notification")
        if res.headers["content-type"].lower().find("application/json") != 0:
            raise JobQuickStart1RPCException("Invalid ContentType in response: " + res.headers["content-type"])
        jres = res.json()
        if "error" in jres:
           raise JobQuickStart1RPCException(jres.get("error").get("message"))
        if not "result" in jres:
            raise JobQuickStart1RPCException(0,"Protocol error in JSON-RPC response - no result nor error node exists")
        return jres["result"]
...
# Following methods are automatically generated
    ...
    def echo(self, value):
        """
        Invokes [function Echo(const value: string): string]
        """
        req = self.__new_req("Echo", "request")
